...
import me.cometkaizo.command.arguments.*;
...
public abstract class CommandNode {
    ...
    private static List<CommandNode> buildSubNodes(Builder builder) {
        return CollectionUtils.map(builder.subNodes, CommandNode.Builder::build);
    }
     ...
     */
    public final boolean match(CommandContext context) throws CommandSyntaxException {
        if (!matchImpl(context)) return false;

        if (hasSubNodes()) return subNodesMatch(context);
        else if (!context.args.hasNext()) return true;
        else {
            context.problems.add(trailingArgsError(context));
            return false;
        }
    }
    ...
    private boolean subNodesMatch(CommandContext context) throws CommandSyntaxException {
        ...
        try {
            ...
            context.problems.add(noMatchError(context));
    ...
    protected Diagnostic.Error trailingArgsError(CommandContext context) {
        ...
        return new Diagnostic.Error(null,
    ...
    public abstract static class Builder {
        ...
        public <T extends Builder> T then(T node) {
            if (!subNodes.isEmpty()) throw new IllegalArgumentException("This node already has sub-nodes: " + subNodes);
