...
import static me.cometkaizo.io.data.DataEnd.endData;
...
public class CompoundData implements Data {
    ...
    public Optional<CompoundData> getCompound(String key) {
        return get(key, CompoundData.class);
    }

    public Optional<ListData> getList(String key) {
        return get(key, ListData.class);
    }

    private <T, R> Optional<R> get(String key, Class<? extends T> expectedDataType, Function<T, R> valueFunction) {
        var data = get(key);
        if (data.isEmpty()) return Optional.empty();
        if (!expectedDataType.isAssignableFrom(data.get().getClass())) {
            throw new ClassCastException("Incorrect data type for key '" + key + "'; expected " + expectedDataType + ", found " + data.getClass());
        }
        return Optional.ofNullable(valueFunction.apply(expectedDataType.cast(data.get())));
    }

    private <T> Optional<T> get(String key, Class<? extends T> expectedDataType) {
        var data = get(key);
        if (data.isEmpty()) return Optional.empty();
        if (!expectedDataType.isAssignableFrom(data.get().getClass())) throw new ClassCastException("Incorrect data type for key '" + key + "'; expected " + expectedDataType + ", found " + data.getClass());
        return Optional.of(expectedDataType.cast(data.get()));
    }
